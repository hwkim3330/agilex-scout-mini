...
> **This session has since been run, and it did not answer its question.** Not one
> CAN frame arrived, in any wiring configuration, and the question of which
> protocol generation the vehicle speaks is still open. What the session did
> produce is in `CAN.md`: the adapter has since been proven good on its own, the
> chassis has been proven alive over RS232, and the untested segment narrowed to
> the wire between them. Read `CAN.md` first. This page is the procedure, kept
> because the procedure was not what failed.
>
> Two things below are out of date. The adapter used was a CANable 2.0 on the PC,
> not a PEAK PCAN-USB through the router, so the `kmod-can-usb-peak` step does not
> apply. And the connections were temporary contacts rather than soldered joints,
> which is now the leading suspect.

The previous edition of this page was the first-flash runbook. That session
happened on 2026-08-13; its results and the flashing procedure are in the router
repository, in
[`doc/VERIFY.md`](https://github.com/hwkim3330/a3004-sensor-bridge/blob/main/doc/VERIFY.md)
and
[`doc/BRINGUP.md`](https://github.com/hwkim3330/a3004-sensor-bridge/blob/main/doc/BRINGUP.md).
...
```sh
tools/flash-router  # in the router repository            # checks the image, then explains
```
